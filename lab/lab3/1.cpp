...
void MAX_FIXUP(TNode *root_now, TNode *nil)
{
    if (root_now == nil)
    {
        return;
    }
    MAX_FIXUP(root_now->left, nil);
    MAX_FIXUP(root_now->right, nil);
    root_now->max = max(root_now->max, root_now->left->max);
    root_now->max = max(root_now->max, root_now->right->max);
}
